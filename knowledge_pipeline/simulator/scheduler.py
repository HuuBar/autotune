...
- E8 账本写回以"假说终局裁决"为记账粒度（03 文档第三部分口径：账本按
  假说记账）。**粒度裁决（已回写 SPEC §4.2，终审 Major-2）**：01 §4-E8
  字面为节点级"写回完成前不得解锁下游"，本实现仅在假说边界保证有序——
  同假说内 A1→A2 等中间动作链是"先执行后写回"（账本在假说终局裁决时
  一次性落账）；跨假说下游 requires 某假说非末位动作时同理。该偏离由
  SPEC §5 账本词表只支持假说级记录所迫，属有记录的工程裁决而非语义遗漏。
  写回失败 → 假说判 blocked 并冻结其动作下游。
